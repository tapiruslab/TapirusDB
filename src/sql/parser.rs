...
fn parse_identifier_or_keyword(tokens: &[Token], cursor: &mut usize) -> Result<String> {
    match get_token(tokens, cursor)? {
        ...
        Token::Rank => Ok("rank".to_string()),
        Token::DenseRank => Ok("dense_rank".to_string()),
        Token::RowNumber => Ok("row_number".to_string()),
        Token::Ntile => Ok("ntile".to_string()),
        Token::Lag => Ok("lag".to_string()),
        Token::Lead => Ok("lead".to_string()),
        Token::Rows => Ok("rows".to_string()),
        Token::Over => Ok("over".to_string()),
        Token::Partition => Ok("partition".to_string()),
        Token::Algorithm => Ok("algorithm".to_string()),
